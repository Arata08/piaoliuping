<template>
  ...
  <view class="empty_box" v-else>
    <uni-icons type="chatboxes" size="200" color="#f0f0f0"></uni-icons>
    <view>
      <span class="txt"> 这里是一片荒草地, </span>
      <span class="txt click">说点什么...</span>
    </view>
  </view>
  <!-- 评论弹窗 -->
  <uni-popup ref="cPopupRef" type="bottom" @change="popChange">
    <view class="c_popup_box">
      <view class="reply_text">
        ...
        <span v-else class="text_main" v-if="!isMine">发表新评论</span>
      </view>
      <view class="content">
      ...
      switch (props.deleteMode) {
        case "bind":
          ...
          emit(
            ...
            (res) => {
              uni.hideLoading();
              c_data.user_content = "当前评论内容已被删除";
            }
          );
          break;
        case "only":
